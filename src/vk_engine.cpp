...
void VulkanEngine::init_pipelines(){
    ...
    skybox = std::make_unique<Skybox>(_device, _gpuSceneDataDescriptorLayout,_allocator, _drawImage, _depthImage);
    _mainDeletionQueue.push_function([&](){
      skybox->clear();  
    });
...
void VulkanEngine::draw_geometry(VkCommandBuffer cmd){
    ...
    *shadowUniformData = shadowSceneData;
    ...
    writer.write_image(3, skybox->_cubeMapHDRTexture.imageView, _defaultCubeSampler, VK_IMAGE_LAYOUT_SHADER_READ_ONLY_OPTIMAL, VK_DESCRIPTOR_TYPE_COMBINED_IMAGE_SAMPLER);
    writer.write_image(4, skybox->_irradianceMapTexture.imageView, _defaultCubeSampler, VK_IMAGE_LAYOUT_SHADER_READ_ONLY_OPTIMAL, VK_DESCRIPTOR_TYPE_COMBINED_IMAGE_SAMPLER);
    writer.write_image(5, skybox->_brdfLUTTexture.imageView, _defaultSamplerLinear, VK_IMAGE_LAYOUT_SHADER_READ_ONLY_OPTIMAL, VK_DESCRIPTOR_TYPE_COMBINED_IMAGE_SAMPLER);
...
//keeping track of the state of the draw pipeline so repeats don't happen 
    ...
    skybox->render_skybox(this, cmd, renderInfo,get_current_frame(),sceneData);
